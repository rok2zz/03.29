...
/* Final  :   마지막 
      - 필드명, 지역변수명  : 상수 ,   final double PI = 3.14 ; 
			: 상수는 값을 변경할 수 없다.
			: 클래스영역 내의 상수 영역에 값이 복사 , 객체나 메소드가 사라질 경우에도 호출이 사용이 가능   
			
      - 메소드명 : 오버라이딩이 불가한 메소드 (부모 클래스에서 자식 클래스에서 메소드 오버라이딩이 불가)
      		: final void print(){} 
...
class A1 {
	...
	final int B = 5 ; 	//상수명 대문자로 사용. 값을 변경 할수 앖다. 
	A1() {}
}
class A2 {
	int a; 
